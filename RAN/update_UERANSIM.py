...
if (retour.count('Running')==nb_pods):
    ...
    if (ok1>0 and ok2>0):
        ...
        with open(r'RAN/OAI-gnb.yaml', 'r') as file:
            data = file.read()
            file.close()
        data = data.replace("xxx", str(ip_adress))
        data = data.replace("yyy", str(amf_ip))
        with open(r'RAN/UERANSIM/build/OAI-gnb.yaml', 'w') as file:
            file.write(data)
            file.close()
        #update OAI-ue file for UERANSIM 
        with open(r'RAN/OAI-ue.yaml', 'r') as file:
            data = file.read()
            file.close()
        data = data.replace("xxx", str(ip_adress))
        data = data.replace("yyy", namespace)
        with open(r'RAN/UERANSIM/build/OAI-ue.yaml', 'w') as file:
            file.write(data)
            file.close()
    else:
        print("Core is not ok")