...
if (retour.count('Running')==nb_pods):
    ...
    if (ok1>0 and ok2>0):
        ...
        while (i<number_of_UEs):  
            slice["ok"][i]=0
            sim["a"]="0"
            filesim = open('RAN/text_files/sim.txt', 'w+')
            filesim.write(json.dumps(sim))
            filesim.close()
            start=time.time()
            end=time.time()
            slice["processus"][i]=subprocess.Popen(['python3', 'RAN/UE.py', str(i)])
            time.sleep(1)
            while ((end-start)<20 and sim["a"]=="0"):
                try:
                    filesim = open('RAN/text_files/sim.txt', 'r')
        ...
        while (i < number_of_UEs):
            if (slice["ok"][i]==1):  
                port=port+1
                slice["iperf_processus"][i]=subprocess.Popen('exec ./RAN/UERANSIM/build/nr-binder '+ str(slice["ip"][i]) + ' iperf3 -c '+ str(ip2)+ ' -p '+str(port)+' -i 1 -t '+str(1)+" 2>&1 | tee RAN/text_files/iperf3.txt",shell = True, stdout = subprocess.PIPE, encoding = 'ascii')
                output = slice["iperf_processus"][i].communicate()[0]
                with open('RAN/text_files/iperf3.txt', 'r') as f:
                ...
                while ("refused" in last_line):
                    ...
                    with open('RAN/text_files/iperf3.txt', 'r') as f:
                        last_line = f.readlines()[-1]
                slice["iperf_processus"][i]=subprocess.Popen('exec ./RAN/UERANSIM/build/nr-binder '+ str(slice["ip"][i]) + ' iperf3 -c '+ str(ip2)+ ' -p '+str(port)+' -i 1 -t '+str(seconds),shell = True, stdout = subprocess.PIPE, encoding = 'ascii')
            i=i+1           
